...
public class DepositAccountNotFoundException extends AbstractPlatformResourceNotFoundException {
    ...
    public DepositAccountNotFoundException(final DepositAccountType accountType, final Long id) {
        super("error.msg." + accountType.getCode().toLowerCase(Locale.ENGLISH) + ".id.invalid",
                StringUtils.capitalize(accountType.toString().toLowerCase(Locale.ENGLISH)) + " account with identifier " + id
                        + " does not exist",
                id);
    }
